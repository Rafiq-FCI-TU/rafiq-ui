  ...
  const handleEmailSubmit = async (values: { email: string }) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        setErrors({
          email: response.data?.message || "Error sending reset link",
        });
      }
    } catch (error) {
  ...
  const handleOtpSubmit = async (otpValue: string) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        setErrors({
          otp: response.data?.message || "Invalid verification code",
        });
      }
    } catch (error) {
      if (axios.isAxiosError(error) && error.response) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 font-sans p-4 lg:p-8">
      <div className="w-full max-w-5xl flex bg-white rounded-[32px] shadow-xl overflow-hidden min-h-[700px]">
        {/* Left side: Branding */}
        <BrandingSection
          imageSrc="/login_photo.jpeg"
          imageAlt="Child playing with blocks"
        />
            ...
            {step === "otp" && (
              <div className="space-y-6">
                ...
                <div className="flex justify-center mb-2">
                  {isLoading && (
                    <Loader2 className="w-6 h-6 animate-spin text-[#188147]" />
                  )}
                </div>
