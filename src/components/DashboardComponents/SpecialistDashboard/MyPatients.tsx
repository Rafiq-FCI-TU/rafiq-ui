  ...
  useEffect(() => {
    ...
    const fetchPatients = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error('Failed to fetch specialist patients');
    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-shadow duration-300 mb-10">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900 leading-tight">My Patients</h2>
            <p className="text-xs text-gray-500 font-medium">{patients.length} children tracked</p>
          </div>
        </div>
        <button
          type="button"
      ...
      {!isLoading && !error && patients.length > 0 && (
        <div className="space-y-4">
          {patients.map((patient) => (
            <div key={patient.patientId} className="flex items-center gap-6 p-3 rounded-xl hover:bg-gray-50 transition-all group cursor-pointer border border-transparent hover:border-gray-100">
            ...
            <div className="flex-1 flex flex-col gap-2">
               <div className="flex justify-between items-center mb-0.5">
                 <span className="font-bold text-base text-gray-900 group-hover:text-green-600 transition-colors">{patient.fullName}</span>
               ...
               <span className="text-[11px] text-gray-500 flex items-center gap-1.5 font-medium">
                 <CalendarDays className="w-3.5 h-3.5" /> Assigned {formatDate(patient.assignedAt)}
               </span>
               <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                 <div className="h-full bg-linear-to-r from-green-500 to-emerald-600 rounded-full transition-all duration-1000 group-hover:h-3" style={{ width: `${Math.max(0, Math.min(100, patient.totalScore))}%` }}></div>
               </div>
            </div>
          </div>
          ))}
